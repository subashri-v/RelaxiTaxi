...
# --- END MODIFIED ---
    ...
    if st.button("Go to Booking Page"):
        st.switch_page("pages/rider/book_ride.py")
    st.stop()

if st.button("Rider History"):
...
# On ride completion (customer side)
if progress >= 1.0:
    rider_id = st.session_state.user_id
    driver_name = booking['driver']  # or driver_id if you save it
    ac = "AC" in booking['type']
    add_ride(
        rider_id=rider_id,
        start=data['start'],
        end=data['end'],
        ...
        fare=booking['fare'],
        ac=ac,
        driver_id=None  # or driver_id if available
    )